...
export default function Products() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    fetch(`${window.baseUrl}products/new`, {
      method: 'POST',
      body: JSON.stringify({
        name: name,
...
const editcategory= async (id) => {
  // console.log(id)
  await fetch(`${window.baseUrl}products/${id}`)
    .then((res) => {
      if (res.status === 200) {
        return res.json()
      }
    ...
    .then(data => {
      setEditId(data.products._id)
      setEditName(data.products.name)
      setEDitdescription(data.products.description)
      setEDitcategory(data.products.category)
      setEditShow(true)
    }
    )
    .catch((error) => {
...
const ViewCategories= (id) => {
  // console.log(id)
  fetch(`${window.baseUrl}products/${id}`)
    .then((res) => {
      if (res.status === 200) {
        return res.json()
      }
    ...
    .then(data => {
      console.log(data)
      setViewId(data.products._id)
      setViewName(data.products.name)
      setViewdescription(data.products.description)
      setViewshow(true)
    }
    )
    .catch((error) => {
...
const handleEditSubmit = (e) => {
  ...
  fetch(`${window.baseUrl}category/${editId}`, {
    ...
    .then(data => {
      setProducts([...Products.filter((category) => category.category_id !== editId), data.data])
...
const renderPhotos = (source) => {
  ...
  const displayitems = Products.map((item) => {
      return (
        <div className="item-management-categories items_contents" key={item._id}>
            ...
            <div className="item_content">
              <div className='item_element'>
                <div className="item_element_buttons">
                  <button className='item_element_button' onClick={() => ViewCategories(item._id)}><img src={Visible} alt="Visible" /></button>
                  <button className='item_element_button'onClick={()=>editcategory(item._id)} ><img src={Edit} alt="Edit" /></button>
                  <button className='item_element_button' onClick={()=>deletecategory(item._id)}><img src={Delete} alt="Delete" /></button>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <Modal show={show} setShow={setShow} title={"Add new Category"}>
        ...
        </div><div className='input_group'>
          <label htmlFor="" className='input_field_label'>Description</label>
          <input className='input_field' type="text" placeholder='Stock'  onChange={(e) => setStock(e.target.value)} />
        </div>
        {/* <div className='input_group'>
          <div>
